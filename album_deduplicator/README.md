...
- מסך ההגדרה הודק למבנה קומפקטי יותר: שתי שורות roots פתוחות כברירת מחדל, הוספת שורות נשארת זמינה, root מועדף חזר למסך הראשי, וההגדרות המתקדמות המקוריות (`force rescan`, `full hash`, `Gemini`) נמצאות שוב תחת section מתקדם קצר.
