...
- סדר ההעדפה לשמירה נקבע כעת לפי סדר תיקיות הסריקה במסך ההגדרה: תיקייה עליונה עדיפה על זו שמתחתיה, גם כאשר ההשוואה היא בין תתי-תיקיות שלהן. ברירת המחדל היא להשתמש בסדר הזה, ואפשר לכבות זאת מהמסך אם רוצים לבחור לפי איכות בלבד.
...
- במסך ההגדרה אפשר להזין מפתח API של `Gemini` מתוך ההגדרות המתקדמות. המפתח נשמר מקומית ב-user-data לשימוש בסריקות הבאות, ואפשר גם להמשיך להשתמש במשתנה הסביבה `GEMINI_API_KEY`.
...
- סביבת ה-review מאפשרת כעת בחירת keeper מהירה לפי מספר העותק: לחיצה על תג המספר בכרטיס או הקשת `1`/`2`/... בוחרת מיד את העותק לשמירה ומעדכנת את סימוני המחיקה ללא כפתור אישור נוסף; מקש רווח מבטל את ההכרעה ושומר את כל העותקים.
...
- `Gemini` הוא enhancement בלבד לזוגות גבוליים:
  ...
  - מפתח ה-API יכול להגיע ממסך ההגדרה, שבו הוא נשמר מקומית להפעלות הבאות, או ממשתנה הסביבה `GEMINI_API_KEY`
...
- כאשר מנגנון ההעדפות פעיל, עותק שנמצא תחת root מוקדם יותר בסדר מקבל הכרעה אוטומטית לשמירה מול עותק תחת root נמוך יותר, גם כאשר איכות העותקים זהה או כאשר הנתיב נבחר בצורה שקולה אך לא מנורמלת. כאשר המנגנון כבוי, בחירת ה-keeper חוזרת להתבסס על איכות בלבד.
