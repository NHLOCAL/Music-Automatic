...
- מסך ההגדרה הודק למבנה קומפקטי יותר: שתי שורות roots פתוחות כברירת מחדל, הוספת שורות נשארת זמינה, והגדרות הסריקה החשובות נשארות גלויות בלי עומס.
...
- נגן ההשוואה בסביבת ה-review מוצג כעת כבאנר צף קומפקטי שמופיע רק בזמן השמעה, עם ציר זמן מעוצב ושליטה מהירה ב-`play/pause` ובסגירה.
...
- מחיקה אוטומטית מוצעת רק כאשר:
  ...
  - הזוג/הקבוצה הם `identical_by_hash` או `final_score > 90`
