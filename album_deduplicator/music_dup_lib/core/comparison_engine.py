...
class ComparisonEngine:
    ...
    def _get_adjusted_weights(self, base_weights: Dict[str, float], hashing_enabled: bool) -> Dict[str, float]:
        ...
        if not hashing_enabled:
            ...
            # Also remove or adjust weight for other_files_similarity if hashing is a key component of it
            # For now, other_files_similarity will still be calculated based on size and name if hashing is off.
            # If enable_hashing is false, the hash part of other_files_similarity just won't contribute.
    ...
    def compare_two_folders(self, folder1: FolderInfo, folder2: FolderInfo) -> Optional[FolderComparisonResult]:
        ...
        if self.enable_hashing and folder1.file_hashes_present and folder2.file_hashes_present:
            ...
            if file_hash_similarity == 1.0:
                ...
                for key in self.weights:
                    if key != 'other_files_similarity' and key not in similarity_scores: # Don't overwrite already calculated other_files_similarity
        ...
        additional_metadata_matches: Dict[str, int] = defaultdict(int) # Ensure type
        ...
        return FolderComparisonResult(
            ...
            is_identical_by_hash=is_identical_by_hash # This is true if music files were identical by hash
        )