...
- `src/App.jsx`
  ...
  - הצגת tabs:
    ...
    - `דורש סקירה`
    - `כל התוצאות`
  - מסך `setup` נשאר קומפקטי עם שתי שורות roots פתוחות כברירת מחדל, בחירה native מרובת תיקיות ב-Desktop, וטוגלים ישירים ל-`Full Hash Scan` ו-`Gemini`.
  - שינוי keeper ידני.
...
- כל שיר נשאר ניתן להשמעה ישירה מתוך העותק שלו, עם נגן docked יחיד שמופיע רק בזמן השמעה
